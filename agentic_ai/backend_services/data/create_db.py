#!/usr/bin/env python3  
"""  
Creates   provizio.db   (SQLite) plus   customer_scenarios.md
...
def populate_data(conn: sqlite3.Connection, markdown_file="customer_scenarios.md"):  
    ...
    promotion_data = [  
        (  
            product_ids["Provizio Mobile Plan"],  
        ...
        (  
            product_ids["Provizio Internet Plan"],  
    ...
    sc1_sub = add_subscription(  
        sc1_cust,  
        product="Provizio Internet Plan",  
        status="active",  
        roaming=0,  
...
def main():  
    with contextlib.closing(sqlite3.connect(DB_NAME)) as conn:  
        create_tables(conn)  
        populate_data(conn)  
    print("✅  provizio.db generated and customer_scenarios.md exported.")
